...
anti_aim[7] = {
	...
	switch_k = ui_new_hotkey("AA", "Anti-aimbot angles", "Manual Invert"),
	
	ind_set = ui_new_multiselect("AA", "Other", "Anti-aimbot Visuals", {"Anti-aim"}),
...
local function handle_menu(hide_all)
    ...
    for i=1, 6 do
		...
		if ui_get(anti_aim[i].bodyyaw) == "Manual Invert" and ui_get(anti_aim[i].enable) then
			sk = true
		end
	end
	...
	ui_set_visible(anti_aim[7].switch_k, ui_get(anti_aim[var.active_i].bodyyaw) == "Manual Invert" and show_menu)
...
local function get_best_desync()
	...
	if ui_get(anti_aim[var.p_state].bodyyaw) == "Manual Invert" and not var.auto_rage then
	...
	if #enemies == 0 then
		...
		if not var.auto_rage then
			...
			elseif ui_get(anti_aim[var.p_state].bodyyaw) == "Jitter" then
				var.best_value = 0
			elseif ui_get(anti_aim[var.p_state].bodyyaw) == "Manual Invert" then
    ...
    if var.bestenemy ~= nil and var.bestenemy ~= 0 and entity_is_alive(var.bestenemy) then
		...
		elseif not var.auto_rage and ui_get(anti_aim[var.p_state].bodyyaw) == "Jitter" then
			var.best_value = 0
		elseif not var.auto_rage and ui_get(anti_aim[var.p_state].bodyyaw) == "ManualInvert" then
			var.best_value = var.flip_value
		else
			var.best_value = 90
		end
...
local function on_setup_command(c)
	...
	if var.auto_rage then
		...
		if ui_get(anti_aim[var.p_state].aa_mode) == "Custom" then
